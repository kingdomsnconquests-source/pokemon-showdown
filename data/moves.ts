...
export const Moves: import('../sim/dex-moves').MoveDataTable = {
	...
	axekick: {
		...
		pp: 10,
		priority: 0,
		flags: { contact: 1, protect: 1, mirror: 1, metronome: 1, crash: 1 },
		hasCrashDamage: true,
	...
	highjumpkick: {
		...
		pp: 10,
		priority: 0,
		flags: { contact: 1, protect: 1, mirror: 1, gravity: 1, metronome: 1, crash: 1 },
		hasCrashDamage: true,
		onMoveFail(target, source, move) {
			this.damage(source.baseMaxhp / 2, source, source, this.dex.conditions.get('High Jump Kick'));
		},
	...
	spikes: {
		...
		condition: {
			...
			onSwitchIn(pokemon) {
				if (!pokemon.isGrounded() || pokemon.hasItem('heavydutyboots') || pokemon.hasItem('travelerscharm')) return;
	...
	toxicspikes: {
		...
		condition: {
			...
			onSwitchIn(pokemon) {
				...
				} else if (pokemon.hasType('Steel') || pokemon.hasItem('heavydutyboots') || pokemon.hasItem('travelerscharm')) {
					// do nothing
